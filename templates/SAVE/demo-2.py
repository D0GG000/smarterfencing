...
def run_full_pipeline(app, video_path, selections):
    ...
    try:
        ...
        run_3d_lifting(app, output_2d, output_3d)
...
def run_data_extractor(app, video_path, output_dir, selections, job_id):
    ...
    def check_score_light(frame, box):
        if not box:
            return "off"
        x1, y1, x2, y2 = box
        ...
        if cv2.countNonZero(red_mask)/total > 0.03 or cv2.countNonZero(green_mask)/total > 0.03:
            return "on"
        return "off"

    def find_exact_light_on_frame(off_frame, on_frame, score_box):
        """Binary search to find exact frame where light turns on"""
        left = off_frame
        right = on_frame
        ...
        while left <= right:
            ...
            light_state = check_score_light(frame, score_box)
    ...
    while current_frame < total_frames:
        ...
        f1_light = check_score_light(fr, fencer1_light)
        f2_light = check_score_light(fr, fencer2_light)
        ...
        if f1_new_touch or f2_new_touch:
            ...
            first_touch_frame = current_frame

            # Handle first detected touch
            if f1_new_touch:
                exact_frame = find_exact_light_on_frame(search_start, current_frame, fencer1_light)
            ...
            if f2_new_touch:
                exact_frame = find_exact_light_on_frame(search_start, current_frame, fencer2_light)
                log(f"Touch detected for Fencer 2 at exact frame {exact_frame}")
                extract_frames_before_touch(exact_frame, "fencer2")
                touches_found += 1
                prev_f2_light = "on"

            # If only one light triggered, check current frame AND next 30 frames for the other light
            if f1_new_touch != f2_new_touch:  # XOR - only one triggered
                # Start from current_frame (re-check) through next 30 frames
                for check_frame in range(current_frame, min(current_frame + 30, total_frames)):
                    ...
                    if f1_new_touch and prev_f2_light == "off":
                        # We detected f1, check for f2
                        if check_score_light(check_fr, fencer2_light) == "on":
                            # Binary search from before first touch to this frame
                            exact_frame = find_exact_light_on_frame(search_start, check_frame, fencer2_light)
                    ...
                    if f2_new_touch and prev_f1_light == "off":
                        # We detected f2, check for f1
                        if check_score_light(check_fr, fencer1_light) == "on":
                            exact_frame = find_exact_light_on_frame(search_start, check_frame, fencer1_light)
            ...
            current_frame += 150
            # Wait for both lights to turn off
            while current_frame < total_frames:
                cap.set(cv2.CAP_PROP_POS_FRAMES, current_frame)
                ret, fr = cap.read()
                if not ret:
                    break
                f1_off = check_score_light(fr, fencer1_light) == "off"
                f2_off = check_score_light(fr, fencer2_light) == "off"
                if f1_off:
                    prev_f1_light = "off"
                if f2_off:
                    prev_f2_light = "off"
    ...
    return fps
def run_3d_lifting(app, input_2d, output_3d):
    ...
    sequences = []
    for vf in glob.glob(os.path.join(input_2d, "*")):
        if os.path.isdir(vf):
            for tf in glob.glob(os.path.join(vf, "*")):
                if os.path.isdir(tf):
                    sequences.append(tf)
    ...
    # After all sequences are processed, detect handedness for each fencer
    if sequences:
        first_job_id = os.path.basename(os.path.dirname(sequences[0]))
        detect_and_update_handedness(app, output_3d, first_job_id)

def run_prediction(app, path_2d, path_3d, job_id):
    model_path = _get_cfg(app, "MODEL_PATH", "./best_touch.pth")
    ...
    class FencingTouchClassifier(nn.Module):
        def __init__(self, input_dim, num_classes=4):
            ...
            self.fc = nn.Sequential(nn.Linear(96,64), nn.GELU(), nn.Dropout(0.3), nn.Linear(64, num_classes))

        def forward(self, x):
            x = F.gelu(self.embed(x))
            x, _ = self.gru(x)
            w = F.softmax(self.attn(x), dim=1)
            return self.fc(torch.sum(w * x, dim=1))
    ...
    for video_folder in glob.glob(os.path.join(path_3d, "*")):
        if not os.path.isdir(video_folder):
            continue

        job_id = os.path.basename(video_folder)
        ...
        for f in files_3d:
            ...
            try:
                ...
                for i in range(1, 31):
                    fp_path = os.path.join(p2d, f"frame_{i}_keypoints.json")
                    ...
                    sf, tf = scorer_seq[i-1], target_seq[i-1]
                ...
                if model is None:
                    ...
                    model = FencingTouchClassifier(features_arr.shape[-1])
...
def register_demo(app):
    ...
    app.config.setdefault("MODEL_PATH", "./best_touch.pth")
